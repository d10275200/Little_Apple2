...
//    };
    ...
    private void testing(){
        register_flag = 1;
        MainActivity.test_flag = 0;
        if(TypeFragment.fbm != null && TypeFragment.fbm.CheckModelHasTrained()){
...
//        }
    ...
    private void StartTestingAlgo(){
        ...
        SC.StartComputingScore(TypeFragment.fbm.getTopKMainFreqBandTable(), SoundWaveHandler.SAMPLE_RATE, FrequencyBandModel.FFT_LENGTH);
        SC.StartLogging();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if( StrokeClassifier.ACTION_OUTPUT_RESULT_STATE.equals(action) ){
                ...
                if(stroke_type.equals("netplay")){
                    ...
                    lob_count++;
                    T_lob.setText(lob_count);
                }else if(stroke_type.equals("drive")){
                    drive_count++;
                    T_drive.setText(drive_count);
                }else if(stroke_type.equals("drop")){
                    drop_count++;
                    T_drop.setText(drop_count);
                }else if(stroke_type.equals("long")){
                    long_count++;
                    T_long.setText(long_count);
                }else if(stroke_type.equals("smash")){
                    smash_count++;
                    T_smash.setText(smash_count);
                }
