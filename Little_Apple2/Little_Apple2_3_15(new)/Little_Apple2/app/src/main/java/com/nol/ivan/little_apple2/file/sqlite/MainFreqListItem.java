...
public class MainFreqListItem {
    ...
    public boolean Check(){
        Cursor cursor=db.rawQuery("SELECT MAX(_id),match_training_id FROM " + TABLE_NAME , null);
        if (cursor.moveToFirst()) {
            Log.e("TAG", "111 --- " + cursor.getInt(0) + "-----" +cursor.getLong(1));
        }
    ...
    public FreqModel GetFreqModelMax(){
        Cursor cursor=db.rawQuery("SELECT MAX(_id),match_training_id FROM " + TABLE_NAME , null);

        if (cursor.moveToFirst()) {
            Log.e("TAG", "111 --- " + cursor.getInt(0) + "-----" +cursor.getLong(1));
        }
        long training_id = cursor.getLong(1);
        return GetFreqModel(training_id);
    }
